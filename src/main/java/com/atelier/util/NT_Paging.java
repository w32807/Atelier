...
public class NT_Paging {
	...
	private String makeHtml(int totalPage, int currentGroup) {
		...
		if(start != 1) {//그룹번호가 1이 아니다 -> 2페이지보다 뒷 쪽이다.
			...
			sb.append("«");		
			//sb.append("</a></li>");
			//sb.append("<li><a href='" + listName + "?pageNum=" + start + "'>"+start+"<span class='sr-only'>(current)</span></a></li>");
			}
		//<a href = list?pageNum=5>[이전]</a>  ---- 6번 페이지에 있을 때, 이전을 눌러 5번페이지로 넘어가도록 만들기.
			...
			/*
			 * sb.append("<font  class='paging' style='color:red;'>"); //이러한 HTML 코드에 class
			 * 혹은 id를 부여하여, css나 javascript를 적용할 수 있다. //그리고 띄어쓰기 조심하자. sb.append("[" + i
			 * +"]</font>");
			 */
				}
